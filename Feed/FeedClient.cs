...
namespace SujaySarma.Sdk.WikipediaApi.Feed
{
    ...
    public class FeedClient : WikipediaClient
    {
        ...
        public FeaturedEventsOnThisDay? GetFeaturedEvents(DateTime date)
        {
            ...
            string? responseJson = GET($"feed/featured/{date.Year}/{date.Month.ToString().PadLeft(2, '0')}/{date.Day.ToString().PadLeft(2, '0')}");
        ...
        {
            string? responseJson = GET("feed/announcements");
            if (! string.IsNullOrWhiteSpace(responseJson))
            {
