...
namespace SujaySarma.Sdk.WikipediaApi.Transform
{
    ...
    public class TransformationClient : WikipediaClient
    {
        ...
        public async Task<string?> HtmlToWikiTextAsync(string html)
        {
            ...
            {
                RequestUri = new Uri($"{ENDPOINT_BASE_URI}/transform/html/to/wikitext"),
                RequestBodyString = requestContent.ToString()
            };

            HttpResponseMessage responseMessage = await client.CallApiMethod(HttpMethod.Post, $"multipart/form-data; boundary={boundaryName}");
